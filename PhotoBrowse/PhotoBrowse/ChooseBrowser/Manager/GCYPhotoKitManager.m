...
@implementation GCYPhotoKitManager

+ (GCYPhotoKitManager *)sharedPhotoKitManager {
    static GCYPhotoKitManager *photoKitManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        photoKitManager = [[GCYPhotoKitManager alloc] init];
    });
            ...
            if (fetchResult.count != 0) {
                ...
                [imageGroupManager requestImageForAsset:asset targetSize:CGSizeMake(80*scale, 80*scale) contentMode:PHImageContentModeDefault options:opt resultHandler:^(UIImage *result, NSDictionary *info) {
                    ...
                    if (!(assetCollection.assetCollectionSubtype == PHAssetCollectionSubtypeSmartAlbumVideos ) &&
                        ...
                        // 得到一张 UIImage，展示到界面上
                        GCYPhotoGroupModel *groupModel = [[GCYPhotoGroupModel alloc]init];
...
- (NSArray *)getPhotoListWithModel:(GCYPhotoGroupModel *)groupModel {
    ...
    for (NSInteger i = 0; i < fetchResult.count; i++) {
        ...
        if (asset.mediaType == PHAssetMediaTypeImage) {
            GCYPhotoModel *photoModel = [[GCYPhotoModel alloc]init];
